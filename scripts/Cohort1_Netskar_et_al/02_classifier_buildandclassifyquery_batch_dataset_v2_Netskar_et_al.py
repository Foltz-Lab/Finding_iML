#Implementation of Machine learning classifier on Netskar et al., data with dataset_v2 as batch 
#----------------------docker(evelyns2000/foltz_tools:classifier2024)----------------------


## input your variables below:
patient = "pan_cancer"
protein = False
batch = 'dataset_v2' # be sure to specify batch for appropriate read in of vae
# the ref variables are the same for all queries
newdirectory = "batch_dataset_v2"
...
adata = sc.read_h5ad("/storage1/fs1/jennifer.a.foltz/Active/Veda/pan_cancer/profiling/updated_batch_pan_cancer_v4_dataset_v2_PtID_v2_extra_datasets_removal/Malmberg_v4_fil_non10X_Datasetv2_PatientIDv2_nonNKremoval_adata_TcellFiltered.h5ad")

print(adata.obs['sample'].value_counts())
print(adata.obs['patient_ID_v2'].value_counts())
...
print(pd.crosstab(adata.obs['predictionsBBC'],adata.obs['sample'], normalize = 'columns'))

predsbytime = pd.crosstab(adata.obs['predictionsBBC'],adata.obs['sample'], normalize = 'columns') # now stash the data for later in an easy to use format
predsbytime.to_csv(f'{patient}_cumulative_probabilitiesBBCbytime.csv') 

adata.write_h5ad(f'{patient}_eMLclassified_adata_batch_dataset_v2_after_extra_datasets_removed.h5ad')
vae_q.save(f'{patient}withtotalvi_vae_model_withclassifiers_batch_dataset_v2_after_extra_datasets_removed', overwrite=True)